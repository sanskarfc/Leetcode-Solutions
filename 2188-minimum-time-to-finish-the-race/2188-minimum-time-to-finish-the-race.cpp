...
public:
    int minimumFinishTime(vector<vector<int>>& tires, int changeTime, int numLaps) {
        ...
        for (int i = 0; i < n; i++) dp[1] = min(dp[1], tires[i][0]);
        for (int x = 1; x <= numLaps; x++) {
            if (x < 20) {
                for (int i = 0; i < n; i++) dp[x] = min(dp[x], without_change[i][x]);
            }
